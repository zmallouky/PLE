...
public class TP6 {
	...
	public static class TP6Reducer extends Reducer<NullWritable, Text, NullWritable, Text> {
		...
		public void reduce(NullWritable key, Iterable<Text> values, Context context)
			...
			for (Text value : values) {
				String[] data = value.toString().split(",");
				String data1 = TopKMap.get(data[1]);
				if (data1 != null) {
					String[] data2 = data1.toString().split(",");
					// la ville existe déja dans la SortedMap
					if (Integer.parseInt(data2[0]) < Integer.parseInt(data[0]))
						// la population de la nouvelle ville est superieure
						// à la population de la ville existante dans la SortedMap
						TopKMap.replace(data[1], data[0].concat(",").concat(data[1]));

				} else {
					// la ville n'existe pas dans la SortedMap
					TopKMap.put(data[1], data[0].concat(",").concat(data[1]));
				}
				
				context.write(key, new Text(value));

			}
			for (String value : TopKMap.values()) {
			context.write(key, new Text(value));
			}
		}
	}
	...
	public static class TP6Mapper2 extends Mapper<Object, Text, NullWritable, Text> {
		public int k = 0;
	...
	public static class TP6Reducer2 extends Reducer<NullWritable, Text, NullWritable, Text> {
		public int k = 0;
		private SortedMap<Integer, String> TopKMap = new TreeMap<Integer, String>();
